...
def wbar(ni, agents, compcost):
    ...
    return sum(compcost(ni, agent) for agent in agents) / float(len(agents))
...
def schedule(succ, agents, compcost, commcost):
    ...
    jobs = [(round(rank(j), 2), j) for j in jobs]
